...
export function grade(question: ContentQuestion, answer: Answer): boolean | null {
  if (!isAutomatic(question)) return null;
  if (question.type === "numeric") {
    const digits = String(Array.isArray(answer) ? answer[0] : answer)
      .replace(/[٠-٩]/g, (digit) => String("٠١٢٣٤٥٦٧٨٩".indexOf(digit)))
      .replace(/[^٠-٩0-9.-]/g, "");
    const expected = question.answer as { value?: number; accepted?: number[] };
    return [expected.value, ...(expected.accepted ?? [])].includes(Number(digits));
  }
  if (question.type === "multiple_select") {
    if (!Array.isArray(answer) || !Array.isArray(question.answer)) return false;
    return JSON.stringify(answer.map(clean).sort()) === JSON.stringify(question.answer.map(String).map(clean).sort());
  }
